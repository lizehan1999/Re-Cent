...
class BaseProcessor(ABC):
    def __init__(self, config, tokenizer, words_splitter, preprocess_text=False):
        ...
        self.relation_token = config.relation_token
    ...
    @staticmethod
    def get_dict_so(spans_sub: List[Tuple[int, int, str]], spans_ob: List[Tuple[int, int, str]],
                    classes_to_id: Dict[str, int]) -> Dict[Tuple[Tuple[int, int], Tuple[int, int]], int]:
        dict_tag = defaultdict(int)
        assert len(spans_sub) == len(spans_ob)
        for sub, ob in zip(spans_sub, spans_ob):
            assert sub[2] == ob[2]
            if sub[2] in classes_to_id:
                dict_tag[((sub[0], sub[1]), (ob[0], ob[1]))] = classes_to_id[sub[2]]
        return dict_tag
    ...
    def get_negatives(batch_list: List[Dict], sampled_neg: int = 5) -> List[str]:
        relation_types = []
        for b in batch_list:
            types = set([el[-1] for el in b['subject']])
            relation_types.extend(list(types))
        relation_types = list(set(relation_types))
        relation_types = sorted(relation_types)
        random.shuffle(relation_types)
        return relation_types[:sampled_neg]
    ...
    def tokenize_inputs(self, texts, entities):
        ...
        for id, text in enumerate(texts):
            ...
            for ent in entities_:
                input_text.append(self.relation_token)
    ...
    def batch_generate_class_mappings(self, batch_list: List[Dict], negatives: List[str] = None) -> Tuple[
        ...
        for b in batch_list:
            ...
            negs_i = negatives[:len(b["subject"]) * neg_type_ratio] if neg_type_ratio else []

            types = list(set([el[-1] for el in b["subject"]] + negs_i))
            types = sorted(types)
    ...
    def collate_raw_batch(self, batch_list: List[Dict], entity_types: List[str] = None,
                          negatives: List[str] = None) -> Dict:
        if entity_types is None:
            class_to_ids, id_to_classes = self.batch_generate_class_mappings(batch_list, negatives)
            batch = [self.preprocess_example(b["tokenized_text"], b["subject"], b["object"], class_to_ids[i]) for i, b
                     in enumerate(batch_list)]
        else:
            class_to_ids = {k: v for v, k in enumerate(entity_types, start=1)}
            id_to_classes = {k: v for v, k in class_to_ids.items()}
            batch = [self.preprocess_example(b["tokenized_text"], b["subject"], b["object"], class_to_ids) for b in
                     batch_list]
...
class SpanProcessor(BaseProcessor):
    def preprocess_example(self, tokens, subject, object, classes_to_id):
        if len(tokens) == 0:
            tokens = ["[PAD]"]
        ...
        dict_lab_sub = self.get_dict(subject, classes_to_id) if subject else defaultdict(int)
        dict_lab_ob = self.get_dict(object, classes_to_id) if object else defaultdict(int)
        dict_lab_so = self.get_dict_so(subject, object, classes_to_id) if subject else defaultdict(int)
        span_label_sub = torch.LongTensor([dict_lab_sub[i] for i in spans_idx])
        span_label_ob = torch.LongTensor([dict_lab_ob[i] for i in spans_idx])
        # span_label_so = torch.LongTensor([dict_lab_so[i] for i in spans_idx])
        spans_idx = torch.LongTensor(spans_idx)
        valid_span_mask = spans_idx[:, 1] > len(tokens) - 1
        span_label_sub = span_label_sub.masked_fill(valid_span_mask, -1)
        span_label_ob = span_label_ob.masked_fill(valid_span_mask, -1)

        return {
            "tokens": tokens,
            "span_idx": spans_idx,
            "span_label_sub": span_label_sub,
            "span_label_ob": span_label_ob,
            "seq_length": len(tokens),
            "entities_sub": subject,
            "entities_ob": object,
            "sub_ob_pair": dict_lab_so
        }

    def create_batch_dict(self, batch, class_to_ids, id_to_classes):
        tokens = [el["tokens"] for el in batch]
        entities_sub = [el["entities_sub"] for el in batch]
        entities_ob = [el["entities_ob"] for el in batch]
        span_idx = pad_sequence([b["span_idx"] for b in batch], batch_first=True, padding_value=0)
        span_label_sub = pad_sequence([el["span_label_sub"] for el in batch], batch_first=True, padding_value=-1)
        span_label_ob = pad_sequence([el["span_label_ob"] for el in batch], batch_first=True, padding_value=-1)
        seq_length = torch.LongTensor([el["seq_length"] for el in batch]).unsqueeze(-1)
        sub_ob_pair = [el["sub_ob_pair"] for el in batch]
        span_mask_sub = span_label_sub != -1
        span_mask_ob = span_label_ob != -1

        return {
            "seq_length": seq_length,
            "span_idx": span_idx,
            "tokens": tokens,
            "span_mask_sub": span_mask_sub,
            "span_mask_ob": span_mask_ob,
            "span_label_sub": span_label_sub,
            "span_label_ob": span_label_ob,
            "entities_sub": entities_sub,
            "entities_ob": entities_ob,
            "classes_to_id": class_to_ids,
            "id_to_classes": id_to_classes,
            "sub_ob_pair": sub_ob_pair
        }
    ...
    def create_labels(self, batch):
        ...
        for id in range(len(batch['tokens'])):
            ...
            ner_sub = batch['entities_sub'][id]
            ner_ob = batch['entities_ob'][id]
            num_classes = len(classes_to_id)
            spans_idx = [(i, i + j) for i in range(len(tokens)) for j in range(self.config.max_width)]
            dict_lab_sub = self.get_dict(ner_sub, classes_to_id) if ner_sub else defaultdict(int)
            dict_lab_ob = self.get_dict(ner_ob, classes_to_id) if ner_ob else defaultdict(int)
            span_label_sub = torch.LongTensor([dict_lab_sub[i] for i in spans_idx])
            span_label_ob = torch.LongTensor([dict_lab_ob[i] for i in spans_idx])
            spans_idx = torch.LongTensor(spans_idx)
            valid_span_mask = spans_idx[:, 1] > len(tokens) - 1
            span_label_sub = span_label_sub.masked_fill(valid_span_mask, 0)
            span_label_ob = span_label_ob.masked_fill(valid_span_mask, 0)
            labels_one_hot_sub = F.one_hot(span_label_sub, num_classes + 1).float()
            labels_one_hot_ob = F.one_hot(span_label_ob, num_classes + 1).float()
            labels_one_hot_sub = labels_one_hot_sub[:, 1:]
            labels_one_hot_ob = labels_one_hot_ob[:, 1:]
            labels_batch_sub.append(labels_one_hot_sub)
            labels_batch_ob.append(labels_one_hot_ob)

        # Convert the list of tensors to a single tensor
        if len(labels_batch_sub) > 1:
            labels_batch_sub = pad_2d_tensor(labels_batch_sub)
            labels_batch_ob = pad_2d_tensor(labels_batch_ob)
        else:
            labels_batch_sub = labels_batch_sub[0]
            labels_batch_ob = labels_batch_ob[0]

        return labels_batch_sub, labels_batch_ob
    ...
    def tokenize_and_prepare_labels(self, batch, prepare_labels):
        ...
        if prepare_labels:
            labels_sub, labels_ob = self.create_labels(batch)
            tokenized_input['labels_sub'] = labels_sub
            tokenized_input['labels_ob'] = labels_ob
